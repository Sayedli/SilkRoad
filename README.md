...
2. **Create a working config**:
   ...
   ```
   Edit `configs/local.yml` to adjust symbols, strategy parameters, or swap `execution.name` to `ibkr` once you are ready. If your region blocks certain exchanges (e.g., Binance), choose another CCXT venue such as `ccxt:kraken` with `BTC/USD`, or use the `static` feed for offline experiments.
3. **Run a backtest**:
   ```bash
